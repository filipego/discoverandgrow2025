...
test("Text Block gives H3 and H4 a smaller shared heading hierarchy with compact spacing", async () => {
  ...
  assert.match(source, /heading3:[\s\S]*as="h3"[\s\S]*size="lg"[\s\S]*className="mb-3 max-w-2xl font-semibold lg:mb-4"/);
  assert.match(source, /heading4:[\s\S]*as="h4"[\s\S]*size="md"[\s\S]*className="mb-3 max-w-xl font-semibold lg:mb-4"/);
  assert.match(source, /heading5:[\s\S]*as="h5"[\s\S]*size="sm"[\s\S]*className="mb-3 max-w-lg font-semibold lg:mb-4"/);
