...
**Last completed:** Audited the live homepage with PageSpeed Insights (mobile: 71/92/73/91; desktop: 85/92/77/91 for Performance/Accessibility/Best Practices/SEO) and applied code-owned first-pass fixes: prioritized the LCP hero image, responsive Prismic image sizing for cards/store content, accessible footer social-link names, guarded header GSAP initialization, and removed store-slider debug logging. Removed the desktop navigation stagger that could leave every navigation item stuck at `translateY(-20px)` while its container remained centered. The header retains its established client-side settings loading and render timing. A follow-up audit identified remaining contrast, generic link-text, and response-security-header findings; the next local change addresses those app-owned findings without adding a potentially disruptive CSP/COOP policy. The current live audit isolates six remaining low-contrast homepage CTAs. Earlier: polished mobile layout across header, slices, forms, footer, and donation checkout; kept desktop behavior intact.
