...
**Last completed:** Audited the live homepage with a fresh isolated Lighthouse run (mobile: 64/96/79/100 for Performance/Accessibility/Best Practices/SEO). Its only accessibility failure is the existing CTA contrast; preserve the established colors unless a visual change is explicitly approved. Its Best Practices failures are the third-party Stripe and Prismic-preview cookies. Preview is now gated to Next draft mode, Stripe checkout is deferred until a rendered donation form nears the viewport, and the existing LCP hero image receives an explicit high fetch priority. Earlier: prioritized the LCP hero image, responsive Prismic image sizing for cards/store content, accessible footer social-link names, guarded header GSAP initialization, removed store-slider debug logging, and removed the desktop navigation stagger that could leave every navigation item stuck at `translateY(-20px)` while its container remained centered.
