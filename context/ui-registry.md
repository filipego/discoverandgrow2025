...
| Component | Path | Purpose |
| --- | --- | --- |
| `BasicCard` | `src/app/components/Cards/BasicCard.tsx` | Standard Prismic image/text/link card with responsive image sizing; optional text-width, image, and rich-text list-spacing classes; Learn more uses `ButtonLink` |
...
| Component | Path | Purpose |
| --- | --- | --- |
| `NewsletterForm` | `src/app/components/Forms/NewsletterForm.tsx` | Newsletter signup; runs invisible Turnstile verification before the server-side Resend contact/email flow; footer passes `inputId="newsletter-email"` for hash-focus |
...
| `LazyCanvaPlayer` | `src/app/components/LazyCanvaPlayer.tsx` | Lazy Canva design/video embed; accepts public Canva design share links |
...
| `WhatWeDoCategorySlider` | `src/slices/WhatWeDoCategories/WhatWeDoCategorySlider.tsx` | Swiper carousel for one What We Do category; on mobile it uses the full content width, shows a clipped partial next card as the swipe affordance, hides the side arrows, and lets each card fit its own content; on tablet/desktop it shows one complete card plus a clipped partial next card, 640px-capped copy, responsive 16:9 bottom-aligned image areas, list-to-paragraph spacing, and protected space for card shadows; navigation appears when 2+ items |
